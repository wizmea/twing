...
class TwingTokenParserFor extends token_parser_1.TwingTokenParser {
    ...
    checkLoopUsageCondition(stream, node) {
        let self = this;
        if ((node.getType() === node_1.TwingNodeType.EXPRESSION_GET_ATTR) && (node.getNode('node').getType() === node_1.TwingNodeType.EXPRESSION_NAME) && (node.getNode('node').getAttribute('name') === 'loop')) {
    ...
    checkLoopUsageBody(stream, node) {
        if ((node.getType() === node_1.TwingNodeType.EXPRESSION_GET_ATTR) && (node.getNode('node').getType() === node_1.TwingNodeType.EXPRESSION_NAME) && (node.getNode('node').getAttribute('name') === 'loop')) {
            ...
            if ((attribute.getType() === node_1.TwingNodeType.EXPRESSION_CONSTANT) && (['length', 'revindex0', 'revindex', 'last'].indexOf(attribute.getAttribute('value')) > -1)) {
