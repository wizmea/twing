...
const get_attr_1 = require("./node/expression/get-attr");
...
class TwingParser {
    ...
    filterBodyNodes(node, nested = false) {
        ...
        if ((node.getType() === node_1.TwingNodeType.TEXT && !ctype_space_1.ctypeSpace(node.getAttribute('data'))) ||
            ((node.getType() !== node_1.TwingNodeType.TEXT) && (node.getType() !== node_1.TwingNodeType.BLOCK_REFERENCE) && (node.TwingNodeOutputInterfaceImpl) && (node.getType() !== node_1.TwingNodeType.SPACELESS))) {
        ...
        if (!nested && (node.getType() === node_1.TwingNodeType.SPACELESS)) {
        ...
        if (nested && (node.getType() === node_1.TwingNodeType.BLOCK_REFERENCE)) {
        ...
        if (node.TwingNodeOutputInterfaceImpl && (node.getType() !== node_1.TwingNodeType.SPACELESS)) {
            return null;
        }
    ...
    getFunctionNode(name, line, column) {
        switch (name) {
            ...
            case 'attribute':
                ...
                return new get_attr_1.TwingNodeExpressionGetAttr(attributeArgs.getNode(0), attributeArgs.getNode(1), attributeArgs.getNodes().size > 2 ? attributeArgs.getNode(2) : null, template_1.TwingTemplate.ANY_CALL, line, column);
            default:
    ...
    parseSubscriptExpression(node) {
        ...
        if (token.value === '.') {
            ...
            if ((node.getType() === node_1.TwingNodeType.EXPRESSION_NAME) && this.getImportedSymbol('template', node.getAttribute('name'))) {
        ...
        return new get_attr_1.TwingNodeExpressionGetAttr(node, arg, arguments_, type, lineno, columnno);
    }
    ...
    parseTestExpression(node) {
        ...
        if ((name === 'defined') && (node.getType() === node_1.TwingNodeType.EXPRESSION_NAME)) {
    ...
    parseArguments(namedArguments = false, definition = false, allowArrow = false) {
        ...
        while (!stream.test(twig_lexer_1.TokenType.PUNCTUATION, ')')) {
            ...
            if (namedArguments && (token = stream.nextIf(twig_lexer_1.TokenType.OPERATOR, '='))) {
                if (value.getType() !== node_1.TwingNodeType.EXPRESSION_NAME) {
    ...
    checkConstantExpression(node) {
        let self = this;
        if (!(node.getType() === node_1.TwingNodeType.EXPRESSION_CONSTANT || node.getType() === node_1.TwingNodeType.EXPRESSION_ARRAY || node.getType() === node_1.TwingNodeType.EXPRESSION_UNARY_NEG || node.getType() === node_1.TwingNodeType.EXPRESSION_UNARY_POS)) {
            return false;
        }
        for (let [k, n] of node.getNodes()) {
