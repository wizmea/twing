...
class TwingNodeModule extends node_1.TwingNode {
    constructor(body, parent, blocks, macros, traits, embeddedTemplates, source) {
        ...
        this.type = node_1.TwingNodeType.MODULE;
        this.source = source;
    ...
    compileDoGetParent(compiler) {
        if (this.hasNode('parent')) {
            ...
            if (parent.getType() === node_1.TwingNodeType.EXPRESSION_CONSTANT) {
                compiler
                    .raw('.then((parent) => {\n')
                    .indent()
    ...
    compileIsTraitable(compiler) {
        ...
        if (traitable) {
            let nodes;
            if (this.getNode('body').getType() === node_1.TwingNodeType.BODY) {
