...
export class TwingParser {
    ...
    filterBodyNodes(node, nested = false) {
        ...
        if ((node.getType() === TwingNodeType.TEXT && !ctypeSpace(node.getAttribute('data'))) ||
            ((node.getType() !== TwingNodeType.TEXT) && (node.getType() !== TwingNodeType.BLOCK_REFERENCE) && (node.TwingNodeOutputInterfaceImpl) && (node.getType() !== TwingNodeType.SPACELESS))) {
        ...
        if (node.TwingNodeOutputInterfaceImpl && (node.getType() !== TwingNodeType.SPACELESS)) {
            return null;
        }
    ...
    getFunctionNode(name, line, column) {
        switch (name) {
            ...
            case 'attribute':
                ...
                return new TwingNodeExpressionGetAttr(attributeArgs.getNode(0), attributeArgs.getNode(1), attributeArgs.getNodes().size > 2 ? attributeArgs.getNode(2) : null, TwingTemplate.ANY_CALL, line, column);
            default:
